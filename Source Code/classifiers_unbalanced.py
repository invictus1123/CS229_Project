...
# full_set = np.concatenate((pos_examples,neg_examples),axis=0)
# full_features,inds = features.constructAllFeatures(full_set[:,0])
# full_set = np.delete(full_set,inds,0)
# full_features = np.delete(full_features,inds, 0)
# full_labels = full_set[:,1].astype(float)
# np.save('full_features',full_features)
# np.save('full_labels',full_labels)
full_features = np.load('full_features.npy')
full_labels = np.load('full_labels.npy')
TEST_SIZE = 51

train_dev_features_unbalanced, test_features, train_dev_labels_unbalanced, test_labels = train_test_split(
                full_features, full_labels, test_size=TEST_SIZE, stratify=full_labels, random_state = 0)
num_pos = np.sum(train_dev_labels_unbalanced)

#split into positives and negatives; first column contains sequences, second column contains labels
pos_features =train_dev_features_unbalanced[train_dev_labels_unbalanced==1,:]
pos_lbls =  train_dev_labels_unbalanced[train_dev_labels_unbalanced==1]
neg_features = train_dev_features_unbalanced[train_dev_labels_unbalanced==0,:]
neg_lbls = train_dev_labels_unbalanced[train_dev_labels_unbalanced==0]
pos_exs = np.array([pos_features, pos_lbls]).T
neg_exs = np.array([neg_features, neg_lbls]).T

# #randomly subsample 203 antibacterial AMPs to create a balanced mini data set of 406 sequences
# np.random.seed(100)
# np.random.shuffle(neg_exs) 
# neg_subset = neg_exs[0:num_pos, :]
# mini_set = np.concatenate((pos_exs, neg_subset), axis=0)
# mini_features = mini_set[:,0]
# mini_labels = mini_set[:,1]
...
#                 train_dev_features_unbalanced, mini_labels, test_size=TEST_SIZE, stratify=mini_labels, random_state = 0)



print 'train_dev_features size: ', train_dev_features_unbalanced.shape
print 'train_dev_labels size: ', train_dev_labels_unbalanced.shape
...
SVM_linl1_fs.fit(train_dev_features_unbalanced,train_dev_labels_unbalanced)
coeff_linl1 = SVM_linl1_fs.coef_
linl1_onerun_features = train_dev_features_unbalanced[:,coeff_linl1[0,:]!=0]
...
PCA_td_features,_, components = features.performPCA(train_dev_features_unbalanced, n_components = 227)
...
inds, bagged_features = feature_sel.cv_featureSelect(SVM_linl1_fs, train_dev_features_unbalanced, train_dev_labels_unbalanced, DEV_SIZE, n=5)
...
# cv_scores, accstds = cV.crossValidate(train_dev_features_unbalanced, train_dev_labels_unbalanced, DEV_SIZE, n_shuffles = 15, \
#                                 models=classifiers, model_names=names)
# cv_scores_m, accstds_m = cV.crossValidate(linl1_onerun_features, train_dev_labels_unbalanced, DEV_SIZE, n_shuffles = 15, \
#                                 models=classifiers_m, model_names=names_m)
# cv_scores_s, accstd_s = cV.crossValidate(bagged_features, train_dev_labels_unbalanced, DEV_SIZE, n_shuffles = 15, \
#                                 models=classifiers_s, model_names=names_s)
# cv_scores_p, accstd_p = cV.crossValidate(train_dev_features_unbalanced, train_dev_labels_unbalanced, DEV_SIZE, n_shuffles = 15, \
#                                  models=classifiers_PCA, model_names=names_PCA, pca = 1, ncomponents = 227)

# scores = test.score(classifiers, names,  train_dev_features_unbalanced,  train_dev_labels_unbalanced, test_features, test_labels)
# scores_m = test.score(classifiers_m, names_m, linl1_onerun_features, train_dev_labels_unbalanced, linl1_test_features, test_labels)
# scores_s = test.score(classifiers_s, names_s, bagged_features, train_dev_labels_unbalanced, bagged_test_features, test_labels)
# scores_PCA = test.score(classifiers_PCA, names_PCA, PCA_td_features,train_dev_labels_unbalanced, PCA_test_features, test_labels)


# train_features, dev_features, train_labels, dev_labels,_ = split.balTrainUnbalDevSplit(linl1_onerun_features,train_dev_labels_unbalanced)
...
# y_score = SVM_linl2_m.fit(train_dev_features_unbalanced, train_dev_labels_unbalanced).decision_function(test_features)
...
#     inds, bagged_features = feature_sel.cv_featureSelect(SVM_linl1_fs, train_dev_features_unbalanced, train_dev_labels_unbalanced, DEV_SIZE, n=it)
#     print 'Selected features for n = %d: ' % it, bagged_features.shape
#     cv_scores_subsetted,accstds = cV.crossValidate(bagged_features, train_dev_labels_unbalanced, DEV_SIZE, n_shuffles = 15, \
...
#gS.gridSearch(train_dev_features_unbalanced, train_dev_labels_unbalanced, DEV_SIZE)
gS.gridSearch(linl1_onerun_features, train_dev_labels_unbalanced, DEV_SIZE)
gS.gridSearch(bagged_features, train_dev_labels_unbalanced, DEV_SIZE)
#gS.gridSearch(PCA_td_features, train_dev_labels_unbalanced, DEV_SIZE)
